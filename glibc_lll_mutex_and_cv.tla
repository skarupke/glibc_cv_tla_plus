...
fair process (WorkerProc \in WorkerProcs)
{
    start: while (TRUE)
    {
      call lock_mutex();
    wait_for_work:
      while (work_to_do = 0 /\ done /= GeneratorProcs)
      {
        call cv_wait();
      };
    take_work:
...
VARIABLES mutex_index, g1_orig_size_index, wrefs_index, g_signals_index, 
          ...
          cv_spinlock_locked, pc, stack, wait_index, val, wake_index, 
          wake_all_index, old_state_, old_state_u, g, g_ref_index, orig_size, 
          old_state, local_wseq, g1, old_orig_size, old_g1_start, r, s, 
          local_wseq_, g_, seq, signals, local_g1_start, local_wseq_c, g1_, 
          do_futex_wake_, local_wseq_cv, g1_c, do_futex_wake, num_loops
...
ProcSet == (WorkerProcs) \cup (GeneratorProcs)
...
Init == (* Global variables *)
        ...
        /\ pc = [self \in ProcSet |-> CASE self \in WorkerProcs -> "start"
                                        [] self \in GeneratorProcs -> "maybe_generate_work"]
...
check_state(self) == /\ pc[self] = "check_state"
                     ...
                     /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                     ...
                                     work_done, done, spinlock_locked, 
...
futex_wake_all_(self) == /\ pc[self] = "futex_wake_all_"
                         ...
                         /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                         ...
                                         wake_index, old_state_, old_state_u, 
                                         g, g_ref_index, orig_size, old_state, 
                                         local_wseq, g1, old_orig_size, 
...
lock_mutex_start(self) == /\ pc[self] = "lock_mutex_start"
                          ...
                          /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                          ...
                                          g_refs_index, wseq, g1_start, g_size, 
                                          did_switch_g1, g1index, work_to_do, 
                                          work_done, done, cv_spinlock_locked, 
...
check_for_locked(self) == /\ pc[self] = "check_for_locked"
                          ...
                          /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                          ...
                                          g1index, work_to_do, work_done, done, 
                                          spinlock_locked, cv_spinlock_locked, 
                                          wait_index, val, wake_index, 
                                          wake_all_index, old_state_u, g, 
                                          g_ref_index, orig_size, old_state, 
...
dec_refs(self) == /\ pc[self] = "dec_refs"
                  ...
                  /\ UNCHANGED << mutex_index, g1_orig_size_index, wrefs_index, 
                                  ...
                                  work_to_do, work_done, done, spinlock_locked, 
...
cv_wait_inner_loop_done(self) == /\ pc[self] = "cv_wait_inner_loop_done"
                                 /\ IF futexes[g_signals_index[g_[self]]].state = signals[self]
                                       ...
                                       ELSE /\ pc' = [pc EXCEPT ![self] = "cv_wait_loop"]
                                            /\ UNCHANGED futexes
                                 /\ UNCHANGED << mutex_index, 
                                                 g1_orig_size_index, 
                                                 wrefs_index, g_signals_index, 
...
cv_wait(self) == cv_wait_fetch_seq(self) \/ cv_wait_incr_wrefs(self)
                    \/ cv_wait_incr_grefs(self) \/ cv_wait_loop(self)
...
generate_work(self) == /\ pc[self] = "generate_work"
                       ...
                       /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                       wrefs_index, g_signals_index, 
                                       g_refs_index, futexes, wseq, g1_start, 
                                       g_size, did_switch_g1, g1index, done, 
                                       spinlock_locked, cv_spinlock_locked, 
                                       wait_index, val, wake_index, 
...
start(self) == /\ pc[self] = "start"
               ...
               /\ UNCHANGED << mutex_index, g1_orig_size_index, wrefs_index, 
                               ...
                               g1_start, g_size, did_switch_g1, g1index, 
                               work_to_do, work_done, done, spinlock_locked, 
                               cv_spinlock_locked, wait_index, val, wake_index, 
...
wait_for_work(self) == /\ pc[self] = "wait_for_work"
                       /\ IF work_to_do = 0 /\ done /= GeneratorProcs
...
take_work(self) == /\ pc[self] = "take_work"
                   ...
                   /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                   ...
                                   did_switch_g1, g1index, work_done, done, 
                                   spinlock_locked, cv_spinlock_locked, 
...
do_work(self) == /\ pc[self] = "do_work"
                 ...
                 /\ UNCHANGED << mutex_index, g1_orig_size_index, wrefs_index, 
                                 g_signals_index, g_refs_index, futexes, wseq, 
                                 g1_start, g_size, did_switch_g1, g1index, 
                                 work_to_do, done, spinlock_locked, 
                                 cv_spinlock_locked, stack, wait_index, val, 
                                 wake_index, wake_all_index, old_state_, 
                                 old_state_u, g, g_ref_index, orig_size, 
                                 old_state, local_wseq, g1, old_orig_size, 
...
broadcast_all_done(self) == /\ pc[self] = "broadcast_all_done"
