...
-------------------------- MODULE glibc_lll_mutex_and_cv_blog_post --------------------------

EXTENDS Integers, FiniteSets, Sequences
CONSTANT WorkerProcs
CONSTANT GeneratorProcs
...
AllProcs == WorkerProcs \union GeneratorProcs
...
variable g_ref_index = 0;
{
dec_refs:
    g_ref_index := g_refs_index[g];
    futexes[g_ref_index].state := futexes[g_ref_index].state - 2;
    if (futexes[g_ref_index].state = 1)
    {
      clear_wake_up_flag:
        futexes[g_ref_index].state := ClearBit(futexes[g_ref_index].state, 0);
        call futex_wake_all(g_ref_index);
        return;
    }
    else
    {
...
{
    maybe_generate_work:- while (num_loops > 0)
    {
      num_loops := num_loops - 1;
      either
      {
        await work_to_do < Cardinality(WorkerProcs);
        call add_work();
      }
      or
      {
        goto generator_all_done;
      };
    };
  generator_all_done:
...
\* BEGIN TRANSLATION
\* Label futex_wake_all of procedure futex_wake_all at line 74 col 5 changed to futex_wake_all_
\* Label futex_wait of procedure lock_mutex at line 107 col 9 changed to futex_wait_
\* Label check_for_waiters of procedure cv_signal at line 398 col 3 changed to check_for_waiters_
\* Label generator_all_done of procedure broadcast_all_done at line 516 col 5 changed to generator_all_done_
\* Label broadcast_all_done of procedure broadcast_all_done at line 521 col 5 changed to broadcast_all_done_
\* Procedure variable old_state of procedure lock_mutex at line 79 col 10 changed to old_state_
\* Procedure variable old_state of procedure unlock_mutex at line 112 col 10 changed to old_state_u
\* Procedure variable local_wseq of procedure cv_wait at line 281 col 11 changed to local_wseq_
\* Procedure variable g of procedure cv_wait at line 282 col 11 changed to g_
\* Procedure variable local_wseq of procedure cv_signal at line 393 col 11 changed to local_wseq_c
\* Procedure variable g1 of procedure cv_signal at line 394 col 11 changed to g1_
\* Procedure variable do_futex_wake of procedure cv_signal at line 395 col 11 changed to do_futex_wake_
\* Procedure variable local_wseq of procedure cv_broadcast at line 444 col 11 changed to local_wseq_cv
\* Procedure variable g1 of procedure cv_broadcast at line 445 col 11 changed to g1_c
VARIABLES mutex_index, g1_orig_size_index, wrefs_index, g_signals_index, 
          ...
          g1index, work_to_do, num_work_started, work_done, all_done, pc, 
          stack, wait_index, val, wake_index, wake_all_index, old_state_, 
          old_state_u, g, g_ref_index, orig_size, old_state, local_wseq, g1, 
...
Init == (* Global variables *)
        ...
        /\ pc = [self \in ProcSet |-> CASE self \in GeneratorProcs -> "maybe_generate_work"
                                        [] self \in WorkerProcs -> "start"]
...
wait_for_wake(self) == /\ pc[self] = "wait_for_wake"
                       ...
                       /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                       ...
                                       all_done, wake_index, wake_all_index, 
                                       old_state_, old_state_u, g, g_ref_index, 
...
futex_wake_all_(self) == /\ pc[self] = "futex_wake_all_"
                         ...
                         /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                         ...
                                         g_ref_index, orig_size, old_state, 
...
unlock_mutex_start(self) == /\ pc[self] = "unlock_mutex_start"
                            ...
                            /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                            ...
                                            g_size, did_switch_g1, g1index, 
                                            work_to_do, num_work_started, 
                                            work_done, all_done, stack, 
                                            wait_index, val, wake_index, 
                                            wake_all_index, old_state_, g, 
                                            g_ref_index, orig_size, old_state, 
                                            local_wseq, g1, old_orig_size, 
                                            old_g1_start, r, s, local_wseq_, 
...
dec_refs(self) == /\ pc[self] = "dec_refs"
                  /\ g_ref_index' = [g_ref_index EXCEPT ![self] = g_refs_index[g[self]]]
                  /\ futexes' = [futexes EXCEPT ![g_ref_index'[self]].state = futexes[g_ref_index'[self]].state - 2]
                  /\ IF futexes'[g_ref_index'[self]].state = 1
                  ...
                  /\ UNCHANGED << mutex_index, g1_orig_size_index, wrefs_index, 
                                  ...
                                  work_to_do, num_work_started, work_done, 
                                  all_done, stack, wait_index, val, wake_index, 
...
clear_wake_up_flag(self) == /\ pc[self] = "clear_wake_up_flag"
                            /\ futexes' = [futexes EXCEPT ![g_ref_index[self]].state = ClearBit(futexes[g_ref_index[self]].state, 0)]
                            /\ /\ g_ref_index' = [g_ref_index EXCEPT ![self] = Head(stack[self]).g_ref_index]
                               /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "futex_wake_all",
                                                                        pc        |->  Head(stack[self]).pc,
                                                                        wake_all_index |->  wake_all_index[self] ] >>
                                                                    \o Tail(stack[self])]
                               /\ wake_all_index' = [wake_all_index EXCEPT ![self] = g_ref_index[self]]
...
cv_release_lock(self) == /\ pc[self] = "cv_release_lock"
                         /\ old_state' = [old_state EXCEPT ![self] = futexes[g1_orig_size_index].state]
...
quiesce_and_switch_start(self) == /\ pc[self] = "quiesce_and_switch_start"
                                  ...
                                  /\ UNCHANGED << mutex_index, 
                                                  ...
                                                  g_refs_index, futexes, wseq, 
                                                  g1_start, g_size, 
                                                  did_switch_g1, work_to_do, 
                                                  num_work_started, work_done, 
                                                  all_done, stack, wait_index, 
                                                  val, wake_index, 
                                                  wake_all_index, old_state_, 
                                                  old_state_u, g, g_ref_index, 
                                                  orig_size, old_state, 
                                                  local_wseq, g1, old_g1_start, 
                                                  r, s, local_wseq_, g_, seq, 
                                                  signals, local_g1_start, 
                                                  local_wseq_c, g1_, 
                                                  do_futex_wake_, 
...
quiesce_flip_g1(self) == /\ pc[self] = "quiesce_flip_g1"
                         ...
                         /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                         wrefs_index, g_signals_index, 
                                         g_refs_index, futexes, wseq, g1_start, 
                                         g_size, did_switch_g1, work_to_do, 
                                         num_work_started, work_done, all_done, 
                                         stack, wait_index, val, wake_index, 
                                         wake_all_index, old_state_, 
                                         old_state_u, g, g_ref_index, 
...
cv_wait_prepare_to_block(self) == /\ pc[self] = "cv_wait_prepare_to_block"
                                  /\ futexes' = [futexes EXCEPT ![g_refs_index[g_[self]]].state = futexes[g_refs_index[g_[self]]].state + 2]
                                        ...
                                        /\ IF (futexes[g_signals_index[g_[self]]].state % 2) = 1 \/ seq[self] < (g1_start \div 2)
                                              THEN /\ /\ g' = [g EXCEPT ![self] = g_[self]]
                                                      /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "condvar_dec_grefs",
                                                                                               pc        |->  "cv_wait_done",
                                                                                               g_ref_index |->  g_ref_index[self],
                                                                                               g         |->  g[self] ] >>
                                                                                           \o stack[self]]
                                                   /\ g_ref_index' = [g_ref_index EXCEPT ![self] = 0]
...
cv_wait_done(self) == /\ pc[self] = "cv_wait_done"
                      ...
                      /\ IF futexes'[wrefs_index].state \div 4 = 1
                            ...
                            ELSE /\ pc' = [pc EXCEPT ![self] = "cv_wait_retake_mutex"]
                                 /\ UNCHANGED << stack, wake_all_index >>
                      /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                      wrefs_index, g_signals_index, 
                                      g_refs_index, wseq, g1_start, g_size, 
                                      did_switch_g1, g1index, work_to_do, 
                                      num_work_started, work_done, all_done, 
                                      wait_index, val, wake_index, old_state_, 
                                      old_state_u, g, g_ref_index, orig_size, 
                                      old_state, local_wseq, g1, old_orig_size, 
                                      old_g1_start, r, s, local_wseq_, g_, seq, 
                                      signals, local_g1_start, local_wseq_c, 
...
check_for_waiters_(self) == /\ pc[self] = "check_for_waiters_"
                            ...
                            /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                            ...
                                            wake_index, wake_all_index, 
                                            old_state_, old_state_u, g, 
                                            g_ref_index, old_state, local_wseq, 
                                            g1, old_orig_size, old_g1_start, r, 
                                            s, local_wseq_, g_, seq, signals, 
...
cv_signal_check_for_early_out(self) == /\ pc[self] = "cv_signal_check_for_early_out"
                                       /\ IF ~did_switch_g1[self]
                                             THEN /\ pc' = [pc EXCEPT ![self] = "cv_signal_release_lock"]
                                             ELSE /\ pc' = [pc EXCEPT ![self] = "cv_signal_add_signal"]
                                       /\ UNCHANGED << mutex_index, 
                                                       g1_orig_size_index, 
                                                       wrefs_index, 
...
cv_signal_decrease_size(self) == /\ pc[self] = "cv_signal_decrease_size"
                                 ...
                                 /\ pc' = [pc EXCEPT ![self] = "cv_signal_release_lock"]
                                 /\ UNCHANGED << mutex_index, 
                                                 g1_orig_size_index, 
                                                 wrefs_index, g_signals_index, 
                                                 g_refs_index, futexes, wseq, 
...
cv_signal_release_lock(self) == /\ pc[self] = "cv_signal_release_lock"
...
add_work_start(self) == /\ pc[self] = "add_work_start"
                        ...
                        /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                        ...
                                        g_size, did_switch_g1, g1index, 
                                        work_to_do, num_work_started, 
                                        work_done, all_done, wait_index, val, 
                                        wake_index, wake_all_index, 
                                        old_state_u, g, g_ref_index, orig_size, 
                                        old_state, local_wseq, g1, 
                                        old_orig_size, old_g1_start, r, s, 
...
generate_work(self) == /\ pc[self] = "generate_work"
                       ...
                       /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                       ...
                                       work_done, all_done, wait_index, val, 
                                       wake_index, wake_all_index, old_state_, 
                                       g, g_ref_index, orig_size, old_state, 
                                       local_wseq, g1, old_orig_size, 
                                       old_g1_start, r, s, local_wseq_, g_, 
                                       seq, signals, local_g1_start, 
                                       local_wseq_c, g1_, do_futex_wake_, 
...
notify_worker(self) == /\ pc[self] = "notify_worker"
                       ...
                       /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                       ...
                                       g_size, did_switch_g1, g1index, 
                                       work_to_do, num_work_started, work_done, 
                                       all_done, wait_index, val, wake_index, 
...
generator_all_done_(self) == /\ pc[self] = "generator_all_done_"
                             /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "lock_mutex",
                                                                      pc        |->  "notify_all_done",
                                                                      old_state_ |->  old_state_[self] ] >>
                                                                  \o stack[self]]
...
notify_all_done(self) == /\ pc[self] = "notify_all_done"
                         ...
                         /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                         ...
                                         g_size, did_switch_g1, g1index, 
                                         work_to_do, num_work_started, 
                                         work_done, wait_index, val, 
                                         wake_index, wake_all_index, 
                                         old_state_, g, g_ref_index, orig_size, 
                                         old_state, local_wseq, g1, 
                                         old_orig_size, old_g1_start, r, s, 
                                         local_wseq_, g_, seq, signals, 
                                         local_g1_start, local_wseq_c, g1_, 
...
generator_all_done(self) == /\ pc[self] = "generator_all_done"
                            /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "broadcast_all_done",
                                                                     pc        |->  "Done" ] >>
                                                                 \o stack[self]]
                            /\ pc' = [pc EXCEPT ![self] = "generator_all_done_"]
                            /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                            wrefs_index, g_signals_index, 
                                            g_refs_index, futexes, wseq, 
                                            g1_start, g_size, did_switch_g1, 
...
GeneratorProc(self) == maybe_generate_work(self)
                          \/ generator_all_done(self)
...
start(self) == /\ pc[self] = "start"
               ...
               /\ pc' = [pc EXCEPT ![self] = "lock_mutex_start"]
               /\ UNCHANGED << mutex_index, g1_orig_size_index, wrefs_index, 
                               g_signals_index, g_refs_index, futexes, wseq, 
                               g1_start, g_size, did_switch_g1, g1index, 
                               work_to_do, num_work_started, work_done, 
                               all_done, wait_index, val, wake_index, 
                               wake_all_index, old_state_u, g, g_ref_index, 
                               orig_size, old_state, local_wseq, g1, 
                               old_orig_size, old_g1_start, r, s, local_wseq_, 
                               g_, seq, signals, local_g1_start, local_wseq_c, 
                               g1_, do_futex_wake_, local_wseq_cv, g1_c, 
                               do_futex_wake, num_loops >>
...
wait_for_work(self) == /\ pc[self] = "wait_for_work"
                       /\ IF work_to_do = 0 /\ all_done /= GeneratorProcs
                             ...
                             ELSE /\ pc' = [pc EXCEPT ![self] = "take_work"]
                                  /\ UNCHANGED << stack, local_wseq_, g_, seq, 
                                                  signals, local_g1_start >>
                       /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                       wrefs_index, g_signals_index, 
                                       g_refs_index, futexes, wseq, g1_start, 
                                       ...
                                       g, g_ref_index, orig_size, old_state, 
                                       local_wseq, g1, old_orig_size, 
                                       old_g1_start, r, s, local_wseq_c, g1_, 
                                       do_futex_wake_, local_wseq_cv, g1_c, 
                                       do_futex_wake, num_loops >>
...
take_work(self) == /\ pc[self] = "take_work"
                   ...
                   /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                   ...
                                   did_switch_g1, g1index, num_work_started, 
                                   work_done, all_done, wait_index, val, 
                                   wake_index, wake_all_index, old_state_, g, 
                                   g_ref_index, orig_size, old_state, 
...
do_work(self) == /\ pc[self] = "do_work"
                 ...
                 /\ UNCHANGED << mutex_index, g1_orig_size_index, wrefs_index, 
                                 g_signals_index, g_refs_index, futexes, wseq, 
                                 g1_start, g_size, did_switch_g1, g1index, 
                                 work_to_do, num_work_started, all_done, stack, 
                                 wait_index, val, wake_index, wake_all_index, 
                                 old_state_, old_state_u, g, g_ref_index, 
                                 orig_size, old_state, local_wseq, g1, 
                                 old_orig_size, old_g1_start, r, s, 
                                 local_wseq_, g_, seq, signals, local_g1_start, 
                                 local_wseq_c, g1_, do_futex_wake_, 
                                 local_wseq_cv, g1_c, do_futex_wake, num_loops >>
...
WorkerProc(self) == start(self) \/ wait_for_work(self) \/ take_work(self)
                       \/ do_work(self) \/ worker_done(self)
...
Next == (\E self \in ProcSet:  \/ futex_wait(self)
                               ...
                               \/ cv_broadcast(self) \/ add_work(self)
                               \/ broadcast_all_done(self))
...
Spec == /\ Init /\ [][Next]_vars
        /\ \A self \in GeneratorProcs : /\ WF_vars((pc[self] # "maybe_generate_work") /\ GeneratorProc(self))
                                        ...
                                        /\ WF_vars(condvar_quiesce_and_switch_g1(self))                                        /\ WF_vars(cv_signal(self))
...
ValidState == /\ wseq <= 4 * MaxNumLoops * Cardinality(WorkerProcs) * Cardinality(GeneratorProcs)
              /\ futexes[wrefs_index].state < 8 * (Cardinality(WorkerProcs) + 1)
              /\ (futexes[g_refs_index[0]].state + futexes[g_refs_index[1]].state) <= (2 * Cardinality(WorkerProcs)) + 2 
              /\ (g_size[0] + g_size[1]) <= Cardinality(WorkerProcs) + 1
              /\ (futexes[g_signals_index[0]].state + futexes[g_signals_index[1]].state) <= 20\*(2 * Cardinality(WorkerProcs)) + 2 + (2 * MaxNumLoops)
...
\* Last modified Sat Oct 31 21:12:21 EDT 2020 by malte
